...
module Make (Data : DataTypes) = struct
  ...
  let () = seal glist
  let free = foreign "g_list_free" (ptr_opt glist @-> returning void)

  let free_full =
    foreign "g_list_free_full"
      (ptr_opt glist @-> GDestroy_notify.funptr @-> returning void)
  ...
  let append dllist element =
    let append_raw =
      foreign "g_list_append"
        (ptr_opt glist @-> ptr data @-> returning (ptr_opt glist))
    in
    match dllist with
    | Some _ -> append_raw dllist element
    | None ->
        let dllist' = append_raw dllist element in
  ...
  let prepend dllist element =
    let is_start =
      ...
      | None -> true
      | Some node -> (
          match getf !@node glist_prev with None -> true | Some _ -> false)
    in
    if is_start then
      let prepend_raw =
        foreign "g_list_prepend"
          (ptr_opt glist @-> ptr data @-> returning (ptr_opt glist))
      in
      match dllist with
      | Some _ -> prepend_raw dllist element
      | None ->
          let dllist' = prepend_raw dllist element in
          let () = finalise dllist' in
          dllist'
    else raise (Invalid_argument "The element is not the start of the list")

  let remove =
    foreign "g_list_remove"
      (ptr_opt glist @-> ptr data @-> returning (ptr_opt glist))

  let first =
    foreign "g_list_first" (ptr_opt glist @-> returning (ptr_opt glist))

  let last = foreign "g_list_last" (ptr_opt glist @-> returning (ptr_opt glist))
  let length = foreign "g_list_length" (ptr_opt glist @-> returning uint)
  ...
  let get_previous = function
    | None -> None
    | Some dllist_ptr -> getf !@dllist_ptr glist_prev

  let get_next = function
    | None -> None
    | Some dllist_ptr -> getf !@dllist_ptr glist_next
  ...
  let sort =
    foreign "g_list_sort"
      (ptr_opt glist @-> Comp_func.funptr @-> returning (ptr_opt glist))

  let nth =
    foreign "g_list_nth" (ptr_opt glist @-> int @-> returning (ptr_opt glist))

  let concat =
    foreign "g_list_concat"
      (ptr_opt glist @-> ptr_opt glist @-> returning (ptr_opt glist))

  let foreach dllist f =
    let foreign_raw =
      foreign "g_list_foreach"
        (ptr_opt glist @-> GFunc.funptr @-> returning void)
    in
