...
module String_hash = GLib.Hash_table.Make (struct
  type key = char
...
let test_hash_table_lookup test_ctxt =
  ...
  let () =
    match String_hash.lookup h espagne with
    | Some _ ->
        let msg = "Lookup bad key: this should not have been reached" in
  ...
  | None ->
      let msg = "Lookup good key: this should not have been reached" in
      assert_equal ~msg false true
  | Some capital ->
      let printer s = s in
...
let test_hash_table_lookup_extended test_ctxt =
  ...
  let espagne = str_to_chr_ptr "espagne" in
  let () =
    match String_hash.lookup_extended h espagne with
    | Some _ ->
        let msg = "Lookup bad key: this should not have been reached" in
        assert_equal ~msg false true
    | None -> ()
  ...
  | Some (key, value) ->
      let printer s = s in
      let () =
        assert_equal ~printer (GLib.Core.char_ptr_to_string key) "allemagne"
      in
...
let test_int_hash_table_create_full test_ctxt =
  ...
  let module Int_hash_wth_destroy = GLib.Hash_table.Make (struct
    type key = int
...
let test_int64_hash_table_create_full test_ctxt =
  ...
  let module Int64_hash_wth_destroy = GLib.Hash_table.Make (struct
    type key = int64
    ...
    let key_destroy_func =
      Some (fun v -> counter_key := Int64.add !counter_key !@v)

    let value_destroy_func =
      Some (fun v -> counter_val := Int64.add !counter_val !@v)
  end) in
...
let tests =
  ...
  >::: [
         "Test hash table create/insert/size"
         >:: test_hash_table_create_insert_size;
         ...
         "Test int64 hash table create_full"
         >:: test_int64_hash_table_create_full;
       ]
