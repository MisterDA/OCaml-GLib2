...
module Int_list = GLib.DLList.Make (struct
  type t = int
...
let test_list_int_previous_next test_ctxt =
  ...
  let _ =
    match Int_list.get_data next with
...
let test_list_int_first test_ctxt =
  ...
  | None ->
      let msg = "the first element of the dllist should not be none" in
      assert_equal ~msg false true
  | first -> (
      match Int_list.get_data first with
      | None ->
          let msg =
            "the data of the first element of the dllist should not be none"
          in
          assert_equal ~msg false true
      | Some v -> assert_equal ~printer:string_of_int 1 !@v)
...
let test_list_int_last test_ctxt =
  ...
  | None ->
      ...
      assert_equal ~msg false true
  | last -> (
      match Int_list.get_data last with
      | None ->
          let msg =
            "the data of the last element of the dllist should not be none"
          in
          assert_equal ~msg false true
      | Some v -> assert_equal ~printer:string_of_int 3 !@v)
...
let test_list_int_remove test_ctxt =
  ...
  let length = Int_list.length dllist' in
  let _ =
    Unsigned.(assert_equal ~printer:UInt.to_string UInt.(of_int 2) length)
  in
...
let test_list_int_prepend_invalid_argument test_ctxt =
  ...
  try
    ...
    let msg =
      "prepend on a node that is not the first should raise an exception."
    in
    assert_equal ~msg false true
  with
  | Invalid_argument _ -> assert true
  | _ ->
      let msg = "the exception should be an Invalid_argument" in
      assert_equal ~msg false true
...
let test_list_int_sort test_ctxt =
  ...
  let dllist = Int_list.prepend dllist three in
  let dllist =
    Int_list.sort dllist (fun ptr_a ptr_b ->
        let a = !@ptr_a in
        let b = !@ptr_b in
...
let test_list_int_concat test_ctxt =
  ...
  let length = Int_list.length dllist'' in
  let () =
    Unsigned.(assert_equal ~printer:UInt.to_string UInt.(of_int 6) length)
  in
  let () = assert_node (Int_list.nth dllist'' 0) 1 string_of_int in
  let () = assert_node (Int_list.nth dllist'' 1) 2 string_of_int in
...
let test_list_int_free_full test_ctxt =
  ...
  let module Int_list = GLib.DLList.Make (struct
    type t = int
...
let test_list_int_foreach test_ctxt =
  let dllist = build_dllist () in
  let counter = ref 0 in
  let () = Int_list.foreach dllist (fun v -> counter := !counter + !@v) in
  assert_equal_int 6 !counter

module Char_ptr_list = GLib.DLList.Make (struct
  type t = char
...
let test_list_char_ptr_sort test_ctxt =
  ...
  let sort_fun a b =
    ...
    if len_a = len_b then 0 else if len_a > len_b then 1 else -1
  in
  let dllist = Char_ptr_list.sort dllist sort_fun in
  let last = Char_ptr_list.last dllist in
  let _ =
    match Char_ptr_list.get_data last with
    | None -> assert_failure "The next node should have data"
    | Some v ->
        let str = GLib.Core.char_ptr_to_string v in
...
let tests =
  ...
  >::: [
         ...
         "Dl list of int prepend invalid argument"
         >:: test_list_int_prepend_invalid_argument;
         ...
         "Dl list of char ptr create append length test"
         >:: test_list_char_ptr_append;
         "DL list of char ptr nth test" >:: test_list_char_ptr_nth;
         "DL list of char ptr previous next test"
         >:: test_list_char_ptr_previous_next;
