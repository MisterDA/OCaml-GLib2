...
let test_uint8_constant test_ctxt =
  ...
  assert_equal
    ~printer:(fun x -> Unsigned.UInt8.to_string x)
...
let test_uint16_constant test_ctxt =
  ...
  assert_equal
    ~printer:(fun x -> Unsigned.UInt16.to_string x)
    (Unsigned.UInt16.of_int 65535)
    maxuint16

let test_int32_constant test_ctxt =
  let maxint32 = GLib.Core.c_MAXINT32 in
  assert_equal
    ~printer:(fun x -> Int32.to_string x)
...
let test_uint32_constant test_ctxt =
  ...
  assert_equal
    ~printer:(fun x -> Unsigned.UInt32.to_string x)
    (Unsigned.UInt32.of_string "4294967295")
    maxuint32

let test_int64_constant test_ctxt =
  let maxint64 = GLib.Core.c_MAXINT64 in
  assert_equal
    ~printer:(fun x -> Int64.to_string x)
    9223372036854775807L maxint64

let test_uint64_constant test_ctxt =
  let maxuint64 = GLib.Core.c_MAXUINT64 in
  assert_equal
    ~printer:(fun x -> Unsigned.UInt64.to_string x)
    (Unsigned.UInt64.of_string "18446744073709551615")
    maxuint64

let test_double_constant test_ctxt =
  let e = GLib.Core.c_E in
  assert_equal ~printer:(fun x -> string_of_float x) 2.718282 e
...
let tests =
  ...
  >::: [
         ...
         "Test dynamically loader version constants"
         >:: test_dynamically_loaded_version_constants;
       ]
